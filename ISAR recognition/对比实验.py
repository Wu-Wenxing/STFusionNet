...
EXPERIMENT_NAME = "train_对比实验_序列数据集"
RESULTS_ROOT = "./results"
SAVE_DIR = os.path.join(RESULTS_ROOT, EXPERIMENT_NAME)
MODEL_SAVE_DIR = os.path.join(SAVE_DIR, "best_models")

NUM_WORKERS = 0
TEST_SNR_LIST = [10, 15, 20, 25]

# Data transforms
class AddGaussianNoise:
    """Add Gaussian noise at a specified SNR using deterministic per-image noise."""
    ...
    def _stable_seed(tensor):
        ...
        # Stable per-image seed for deterministic test noise.
...
class SequenceConsistentTransform:
    """Apply the same random augmentation parameters to all frames in a sequence."""

    def __init__(self, train=False, noise_snr=None):
        self.train = train
    ...
    def __call__(self, images):
        ...
        if self.train:
            ...
            # Sample one augmentation configuration and apply it to the whole sequence.
...
def train_stfusion_model(model, train_loader, val_loader, num_epochs=EPOCHS, save_path=None):
    ...
    # Optimize network parameters and the learnable class centers jointly.
    ...
    if save_path is not None:
        ...
        model_basename = os.path.basename(save_path)
        history_filename = model_basename.replace('.pth', '_loss_history.npz')
...
def stratified_split_by_class(samples, class_to_idx, train_ratio=0.6, val_ratio=0.2, test_ratio=0.2, seed=None):
    """Split samples by class into train, validation, and test subsets."""
...
def run_all_experiments():
    ...
    for run_idx in range(NUM_REPEATS):
        ...
        def get_seq_transforms(noise_snr=None):
            ...
            train_t = SequenceConsistentTransform(train=True, noise_snr=None)
            val_t = SequenceConsistentTransform(train=False, noise_snr=None)
            if noise_snr is not None:
                test_t = SequenceConsistentTransform(train=False, noise_snr=noise_snr)
            else:
                test_t = val_t
            return train_t, val_t, test_t
    ...
    np.savez(os.path.join(SAVE_DIR, '对比实验结果_clean_allmetrics.npz'), **clean_save)
    ...
    for snr in TEST_SNR_LIST:
        ...
        np.savez(os.path.join(SAVE_DIR, f'对比实验结果_noise{snr}dB_allmetrics.npz'), **snr_save)

    with open(os.path.join(SAVE_DIR, '对比结果_详细指标.txt'), 'w') as f:
    ...
    print("\n所有详细结果已保存至：")
    print(f"  - 主目录: {SAVE_DIR}")
    print(f"  - 模型目录: {MODEL_SAVE_DIR}")
    print("  - 对比实验结果_clean_allmetrics.npz")
    for snr in TEST_SNR_LIST:
        print(f"  - 对比实验结果_noise{snr}dB_allmetrics.npz")
    print("  - 对比结果_详细指标.txt")
